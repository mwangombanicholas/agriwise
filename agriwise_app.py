...
with tab1:
    ...
    if uploaded:
        ...
        with col_img:
            ...
            st.caption(f"Image: {img_width} × {img_height} pixels")
        ...
        with col_result:
            try:
                ...
                if not os.path.exists(model_path):
                    with st.spinner("📥 Downloading AI model (61MB)... Please wait."):
                ...
                <div class="result-card info">
                    <h3>🔄 Demo Mode</h3>
                    <p>The AI model is currently loading. Here's a sample diagnosis.</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                st.caption("📌 Full AI model will be available in production.")
...
with tab2:
    ...
    # District data
    districts = {
        "Mzuzu": {"rainfall": "Moderate", "temp": "24°C", "humidity": "68%", "alert": "none"},
        "Mzimba": {"rainfall": "Low", "temp": "26°C", "humidity": "55%", "alert": "dry"},
        "Karonga": {"rainfall": "High", "temp": "29°C", "humidity": "65%", "alert": "flood"},
        "Rumphi": {"rainfall": "Moderate", "temp": "23°C", "humidity": "72%", "alert": "none"},
        "Nkhata Bay": {"rainfall": "Very High", "temp": "27°C", "humidity": "70%", "alert": "flood"},
        "Chitipa": {"rainfall": "Moderate", "temp": "22°C", "humidity": "60%", "alert": "none"},
        "Kasungu": {"rainfall": "Low", "temp": "25°C", "humidity": "58%", "alert": "dry"},
        "Lilongwe": {"rainfall": "Moderate", "temp": "26°C", "humidity": "62%", "alert": "none"}
    }
    ...
    if selected:
        ...
        with m4:
            season = "🌱 Growing" if data["rainfall"] in ["Moderate", "High", "Very High"] else "☀️ Dry"
        ...
        alert_messages = {
            "flood": {"message": "🚨 FLOOD ALERT: Heavy rains expected. Move livestock to higher ground and check drainage.", "type": "error"},
            "dry": {"message": "🌵 DRY SPELL ALERT: Irrigate crops if possible. Apply mulch to retain moisture.", "type": "warning"}
        }
        
        alert = alert_messages.get(data["alert"])
        if alert:
            if alert["type"] == "error":
        ...
        with c2:
            st.metric("🌽 Next Planting", "Nov-Dec")
        with c3:
        ...
        st.caption("📡 Weather data: NASA POWER & Malawi Met Department")
...
<div class="footer">
    ...
    <p style="font-size:0.8rem; opacity:0.7;">Model Accuracy: 96% • July 2026</p>
</div>
""", unsafe_allow_html=True)
